...
void createDb(const std::string& worldName)
{
	...
	SQLite::Database db(worldName, SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
	auto createSql = loadTextFile("res/NewMap.sql");
	db.exec(createSql);

	SQLite::Statement query(db,
							"INSERT OR REPLACE INTO properties (ID, WorldSize, WorldHeight) "
							"VALUES (?, ?, ?)");
	query.bind(1, 1);
...
void loadChunk(const SQLite::Database& db, uint32_t x, uint32_t y, uint32_t z)
{
	SQLite::Statement query(
		db, "SELECT block_data FROM chunks WHERE chunk_x = ? AND chunk_y = ? AND chunk_z = ?");
...
{
	auto* world = World::get();
	auto dbPath = WORLDS_PATH + worldName + ".sqlite3";
	...
	for(uint32_t cx = 0; cx < world->worldSize; cx++)
	{
		for(uint32_t cz = 0; cz < world->worldSize; cz++)
		{
			for(uint32_t cy = 0; cy < world->worldHeight; cy++)
			{
				...
				query.bind(2, cy);
				query.bind(3, cz);
				query.bind(4, chunk->blocks.data(), CHUNK_MEMORY_SIZE);
...
void loadWorld(const std::string& worldName)
{
	...
	SQLite::Statement query(db, "SELECT WorldSize, WorldHeight FROM properties WHERE ID = 1");
	uint32_t worldSize = 0, worldHeight = 0;
	if(query.executeStep())
	{
		worldSize = query.getColumn("WorldSize").getUInt();
		worldHeight = query.getColumn("WorldHeight").getUInt();
	}
	else
	{
		std::abort();
...
{
	return fileExists(WORLDS_PATH + worldName + ".sqlite3");
}
} // namespace persistence
} // namespace world
} // namespace vblck