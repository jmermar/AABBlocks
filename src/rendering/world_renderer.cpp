...
{
	auto* deferredBuffers = &Renderer::get()->deferredBuffers;
	deferredBuffers->depthBuffer.transition(
		cmd, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL);
	deferredBuffers->albedo.transition(
		cmd, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
