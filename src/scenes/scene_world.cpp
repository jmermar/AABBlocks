...
void sceneWorld_InitNewWorld(uint32_t worldSize,
							 uint32_t worldHeight)
{
	...
	scene.worldGenerator.world_height =
		worldHeight;
	...
	render::RenderState& renderState)
{
	...
	world->chunkGenerateCommands = std::move(
		scene.worldGenerator.chunksToGenerate);
	...
	if(InputData::isPressed(INPUT_SELECT_UP))
	{
		...
		scene.ui.blockSelect.currentSelect =
			1 +
			(world->blockDatabase.blocks.size() +
			 id - 1) %
				world->blockDatabase.blocks
					.size();
	}
...
bool sceneWorld_DrawGUI()
{
	...
	if(!scene.loaded)
	{
		...
		ImGui::Text(
			"Progress: %.2f",
			scene.worldGenerator.progress *
				100.f);
		return false;
	}
	if(InputData::isPressed(INPUT_MENU))
	{
		InputData::setCaptureMosue(
			!InputData::getCaptureMosue());
	}

	ImGui::Spacing();
	ImGui::Text("Select Block");
	for(uint32_t i = 1;
		i <= world->blockDatabase.blocks.size();
		i++)
	{
		bool select =
			i ==
			scene.ui.blockSelect.currentSelect;
		auto* block =
			world->blockDatabase.getBlockFromId(
				i);
		ImGui::MenuItem(
			block->name.c_str(), 0, &select);
	}
