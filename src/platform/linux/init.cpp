...
namespace vblck
{
System initSystemLinux(const char* win_name,
					   int W,
					   int H)
{
	System system;
	...
	system.window = SDL_CreateWindow(
		win_name, W, H, window_flags);

	vkb::InstanceBuilder builder;

	auto inst_ret =
		builder.set_app_name("VBlck")
	...
	system.debugMessenger =
		vkb_inst.debug_messenger;

	SDL_Vulkan_CreateSurface(system.window,
							 system.instance,
							 nullptr,
							 &system.surface);
...
{
	vkDestroySurfaceKHR(
		system.instance, system.surface, nullptr);
	vkDestroyDevice(system.device, nullptr);

	vkb::destroy_debug_utils_messenger(
		system.instance, system.debugMessenger);
